...
def bot_move():
    ...
    multiple of 4 sticks remaining after its move. Displays a message telling the player how many 
    sticks the bot took.
    ...
    """
    global sticks
    take = sticks % 4  # Calculate the number of sticks to take to leave a multiple of 4
    if take == 0:
        take = 1  # If the number of sticks is already a multiple of 4, take 1 stick
    sticks -= take
    messagebox.showinfo("Bot's Move", f"Bot takes {take} stick(s).")
    update_sticks_label()
    if sticks == 0:
...
def reset_game():
    """
    Resets the game to its initial state with 21 sticks and updates the display.
...
tk.Button(button_frame, text="Take 2", command=lambda: player_move(2), width=10).grid(row=0, column=1)
tk.Button(button_frame, text="Take 3", command=lambda: player_move(3), width=10).grid(row=0, column=2)
